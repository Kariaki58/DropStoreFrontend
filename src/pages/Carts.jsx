          ...
          <div key={index} className="bg-[#004085] flex flex-row max-custom-sm:flex-col sm:w-full justify-between bg-light-gray text-white p-4 rounded-lg shadow-lg">
            ...
            <div className="flex flex-col justify-center mt-5 lg:mt-0 lg:ml-5">
              ...
              <div className="flex items-center gap-3 mt-2">
                ...
                <button onClick={() => handleDecrFromCart(item.productId._id)} className="text-2xl font-bold bg-[#007BFF] text-white p-2 rounded-lg hover:bg-darker-teal">-</button>
              </div>
            </div>
          </div>
        ))}
      </div>
      <div className="bg-light-gray w-40 h-40 bg-[#004085] rounded-full flex flex-col items-center justify-center p-4 text-white fixed bottom-10 right-10 shadow-lg">
        <h1 className="text-lg font-bold">${calculateTotalPrice()}</h1>
        <button onClick={makePayment} className="mt-2 bg-[#007BFF] hover:bg-slate-900 text-white font-semibold py-1 px-4 rounded-xl">
          Checkout
        </button>
      </div>
    </section>
